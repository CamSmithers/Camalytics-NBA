...
all_team_data <- all_team_data %>%
    ...
    mutate(team_missed_shots = team_fga - team_fgm)
```
...
average_team <- all_team_data %>% #Creating data frame with averages of specific statistics for each team
    ...
    summarize(
        ...
        team_missed_shots = round(mean(team_missed_shots, na.rm = TRUE))
    ) %>%
...
opponent_average_team <- all_team_data %>%
    ...
    summarize(
        ...
        opponent_team_missed_shots = round(mean(team_missed_shots, na.rm = TRUE))
    ) %>%
...
## Data Cleaning 6: Adding Descriptive Variables
...
## Data Cleaning 7: Columns Sums and Rates (Overall and Cumulative)
