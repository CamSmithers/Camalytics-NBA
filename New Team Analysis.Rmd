...
all_team_data <- all_team_data %>%
    ...
    select(team, home, everything())
team_temporary_df <- all_team_data %>%
...
average_team_last10 <- all_team_data %>%
    group_by(team_team) %>%
    slice_tail(n = 10) %>%
    summarize(
        team_pts = round(mean(team_pts, na.rm = TRUE), 2), 
...
average_team_full <- average_team %>%
    left_join(average_team_last10, by = c("avg_team_team" = "avg_team_team_last10")) %>%
    mutate(team_weighted_pts = (avg_team_pts * 0.50) + (avg_team_pts_last10 * 0.50)) %>%
    mutate(team_weighted_reb = (avg_team_reb * 0.50) + (avg_team_reb_last10 * 0.50)) %>%
    mutate(team_weighted_ast = (avg_team_ast * 0.50) + (avg_team_ast_last10 * 0.50)) %>%
    mutate(team_weighted_fgpct = (avg_team_fgpct * 0.50) + (avg_team_fgpct_last10 * 0.50)) %>%
    mutate(team_weighted_tspct = (avg_team_tspct * 0.50) + (avg_team_tspct_last10 * 0.50)) %>%
    mutate(team_weighted_offrating = (avg_team_offrating * 0.50) + (avg_team_offrating_last10 * 0.50)) %>%
    mutate(team_weighted_defrating = (avg_team_defrating * 0.50) + (avg_team_defrating_last10 * 0.50)) %>%
    mutate(team_weighted_netrating = (avg_team_netrating * 0.50) + (avg_team_netrating_last10 * 0.50)) %>%
    mutate(team_weighted_tov = (avg_team_tov * 0.50) + (avg_team_tov_last10 * 0.50)) %>%
    mutate(team_weighted_plusminus = (avg_team_plusminus * 0.50) + (avg_team_plusminus_last10 * 0.50))
...
all_team_data <- data.frame(all_team_data, league_average)
```

## Data Cleaning 5: Ranking Team Performances

```{r}
rank_by_offrating <- all_team_data %>%
...
rank_by_offrating_1 <- rank_by_offrating %>%
...
rank_by_defrating <- all_team_data %>%
...
rank_by_defrating_1 <- rank_by_defrating %>%
    arrange(team_collective_defrating_subrank) %>%
...
```{r warning=FALSE}
a <- ggplot(all_team_data, aes(as.factor(team_offrating_tier), team_offrating, fill = as.factor(team_division)))
b <- ggplot(all_team_data, aes(team_team, team_defrating, fill = as.factor(team_winloss)))
