...
all_team_data <- all_team_data %>%
    ...
    mutate(team_game_outcome_3 = ifelse(team_plusminus > 10, 1, 0)) %>%
...
all_team_average <- all_team_data %>%
    ...
    fill(
        ...
        #Other Statistics
        team_cummean_reb, team_cummean_tov, team_cummean_netrating, .direction = "down"
        ) %>%
    dplyr::select(
        team_team, team_date,
        #Offensive Statistics
...
all_opponent_average <- all_team_data %>%
    ...
    fill(
        ...
        #Other Statistics
        opponent_cummean_reb, opponent_cummean_tov, opponent_cummean_netrating, .direction = "down"
        ) %>%
    dplyr::select(
        team_team, team_date,
        #Offensive Statistics
...
all_team_data <- all_team_data %>%
    ...
    mutate(
        ...
        team_cumsum_blowout_win = cumsum(ifelse(is.na(team_game_outcome_3), 0, team_game_outcome_3)),
        ...
        team_cumrate_blowout_win = ifelse(team_cumsum_game == 0, NA, round(team_cumsum_blowout_win / team_cumsum_game, 3)),
...
```{r}
winloss_model_df <- as.data.frame(winloss_model_summary$coefficients) %>%
    rownames_to_column(var = "Variable") %>%
    as_tibble() %>%
    filter(`Pr(>|z|)` < .05)

all_team_data <- all_team_data %>%
    mutate(team_win_chance = 
               round(100 * (1 / (1 + e^-(
                   (winloss_model_df$Estimate[1] * team_home)
                   + (winloss_model_df$Estimate[2] * team_cummean_offrating)
                   + (winloss_model_df$Estimate[3] * team_cummean_defrating)
                   + (winloss_model_df$Estimate[4] * opponent_cummean_offrating)
                   + (winloss_model_df$Estimate[5] * opponent_cummean_defrating)))), 1))
```

## Outcome 3 (Secondary): Game Outcome Magnitude (w/ Primary Outcome Assumption)

```{r}
blowout_model <- glm(team_game_outcome_3 ~ 1
                 , data = all_team_data, family = "binomial")
...
blowout_model_df <- as.data.frame(blowout_model_summary$coefficients) %>%
    rownames_to_column(var = "Variable") %>%
    as_tibble() %>%
    filter(`Pr(>|z|)` < .05)

all_team_data <- all_team_data %>% 
   mutate(team_blowout_chance = 
               round(100 * (1 / (1 + e^-(
                   (blowout_model_df$Estimate[1] * team_home)
                   + (blowout_model_df$Estimate[2] * team_cummean_offrating)
                   + (blowout_model_df$Estimate[3] * team_cummean_defrating)
                   + (blowout_model_df$Estimate[4] * opponent_cummean_offrating)
                   + (blowout_model_df$Estimate[5] * opponent_cummean_defrating)))), 1))
```
...
```{r}
upset_model_df <- as.data.frame(upset_model_summary$coefficients) %>%
    rownames_to_column(var = "Variable") %>%
    as_tibble() %>%
    filter(`Pr(>|z|)` < .05)

all_team_data <- all_team_data %>%
    mutate(team_upset_chance = case_when(
               team_favored == 1 ~ round(100 * (1 / (1 + e^-(
                   (upset_model_df$Estimate[1]) 
                   + (upset_model_df$Estimate[2] * team_cumrate_upset)))), 1),
               team_favored == 0 ~ NA
               ))
```
