...
```{r}
outcome_margin_model <- lm(team_netrating ~ team_home
...
```{r}
outcome_margin_predicted_values <- predict(outcome_margin_model, newdata = all_team_data)
all_team_data <- cbind(all_team_data, outcome_margin_predicted_values)

#Cleaning Up Probability
all_team_data <- all_team_data %>%
    mutate(outcome_margin_predicted_values = round(outcome_margin_predicted_values))
```
