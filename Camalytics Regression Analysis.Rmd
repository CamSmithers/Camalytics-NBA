...
all_league_average <- all_team_data %>%
    ...
    arrange(team_date) %>%
    mutate(
        league_cummean_pts = round(mean(league_avg_pts), 1),
        league_cummean_pts_allowed = round(mean(league_avg_pts_allowed), 1),
        league_cummean_reb = round(mean(league_avg_reb), 1),
        league_cummean_ast = round(mean(league_avg_ast), 1),
        league_cummean_tov = round(mean(league_avg_tov), 1),
        #Advanced Metrics
        league_cummean_offrating = round(mean(league_avg_offrating), 1),
        league_cummean_defrating = round(mean(league_avg_defrating), 1),
        league_cummean_netrating = round(mean(league_avg_netrating), 1)) %>%
    fill(league_cummean_pts, league_cummean_pts_allowed, league_cummean_reb, 
         league_cummean_ast, league_cummean_tov, 
         league_cummean_offrating, league_cummean_defrating, league_cummean_netrating) %>%
...
all_team_data <- all_team_data %>%
    ...
    mutate(upset_predicted_values = case_when(
        team_favored == 1 ~ upset_predicted_values,
        team_favored == 0 ~ NA
    ))
```
