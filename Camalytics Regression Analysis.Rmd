...
```{r}
blowout_model <- glm(team_game_outcome_3 ~ team_cummean_netrating
                     + opponent_cummean_netrating
                     + team_cumrate_blowout_win
                 , data = all_team_data, family = "binomial")
...
all_team_data <- all_team_data %>% 
   mutate(team_blowout_chance = 
               round(100 * (1 / (1 + e^-(
                   (blowout_model_df$Estimate[1])
                   + (blowout_model_df$Estimate[2] * team_cummean_netrating)
                   + (blowout_model_df$Estimate[3] * opponent_cummean_netrating)
                   + (blowout_model_df$Estimate[4] * team_cumrate_blowout_win)))), 1))
```
