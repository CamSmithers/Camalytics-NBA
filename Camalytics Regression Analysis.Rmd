...
all_team_data <- all_team_data %>%
    mutate(outcome_2_estimates_categories = case_when(
        winloss_predicted_values >= 50 & winloss_predicted_values < 65 & upset_predicted_values >= 50 ~ 0,  # Category 0
        winloss_predicted_values >= 65 & upset_predicted_values >= 50 ~ 1,  # Category 1
        winloss_predicted_values >= 50 & winloss_predicted_values < 65 & 
            upset_predicted_values >= 25 & upset_predicted_values < 50 ~ 2,  # Category 2
        (winloss_predicted_values >= 50 & winloss_predicted_values < 65 & upset_predicted_values < 25) |  # Category 3
        (winloss_predicted_values >= 65 & upset_predicted_values >= 25 & upset_predicted_values < 50) ~ 3,
        winloss_predicted_values >= 65 & upset_predicted_values < 25 ~ 4,  # Category 4
    )) %>%
...
mv_outcome_2 <- mv_all_team_data %>%
    filter(!is.na(upset_predicted_values))

mv_outcome_2_daily <- mv_outcome_2 %>%
...
# Scatter Plot
winloss_plot_3 <- ggplot(mv_all_team_data, aes(x = winloss_predicted_values, y = upset_predicted_values, color = as.factor(outcome_2_estimates_categories))) + 
...
winloss_plot_4 <- ggplot(mv_outcome_2, aes(x = as.factor(outcome_2_estimates_categories), fill = as.factor(team_game_outcome_2))) + 
    geom_bar(position = "dodge", color = "black")
winloss_plot_4

winloss_plot_5 <- ggplot(mv_outcome_2_daily, aes(x = winloss_predicted_values, y = upset_predicted_values, color = team_team)) + 
    geom_vline(xintercept = 65, color = "black", linetype = "dashed") +
    geom_hline(yintercept = 50, color = "black", linetype = "dashed") + 
    geom_hline(yintercept = 25, color = "black", linetype = "dashed") + 
    geom_abline(slope = 25/65, color = "black", linetype = "solid") +
