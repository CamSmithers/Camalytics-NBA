...
daily_team_outcomes <- all_team_data %>%
    select(team_date, team_home, team_team, team_favored, team_opponent, winloss_team_prob, 
           winloss_v3_team_prob, close_win_v3_prob, upset_v2_team_prob) %>%
    filter(team_date == Sys.Date())
```
...
cover_ast_daily_analysis <- all_data %>%
    select(player_date, player_team, player_player, team_opponent, player_gen_player_ast,
           avg_player_ast, player_cover_ast_cumsum_rate, cover_ast_v1_prob) %>%
    filter(player_date == Sys.Date())
```

# Player Models Data Frame

```{r}
daily_player_outcomes <- all_data %>%
    select(player_date, player_team, player_player, team_opponent, cover_pts_v3_prob, 
           cover_reb_v1_prob, cover_ast_v1_prob) %>%
    filter(player_date == Sys.Date())
```
